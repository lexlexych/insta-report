...
export default function Page() {
  ...
  }, [canContinue, go, knowledge, locale, orgName, saving, sphere, step, t, tenant, username]);
  ...
  </section>{!isFinish && !kbEditing ? <footer className="shrink-0 border-t bg-tg-bg p-4"><div className="mx-auto flex w-full max-w-xl gap-3">{hasBack ? <button type="button" onClick={() => void back()} className="rounded-xl bg-tg-secondary-bg px-5 py-4 font-medium">{t('onboardingBack')}</button> : null}<button type="button" disabled={!canContinue || saving} onClick={() => void next()} className="flex-1 rounded-xl bg-tg-button p-4 font-medium text-tg-button-text disabled:opacity-40">{saving ? t('onboardingSaving') : step === 'knowledge' ? t('onboardingComplete') : t('onboardingNext')}</button></div></footer> : null}</main>;
}
