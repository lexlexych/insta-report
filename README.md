...
(`src/lib/kb-templates/{ru,de}/` — на каждом языке системы; показывается на языке интерфейса,
определяемом из языка Telegram); подключение Instagram проходит через заявку с подтверждением
...
## Каналы подключения Instagram

Есть два независимых способа подключения:

- **Meta Direct** — исходный путь: аккаунт добавляется в нашу Meta App, поэтому для пилота
  требуются приглашение тестировщика и одобрение администратора.
- **Zernio** — опциональный путь с подключением в один клик через Meta App Zernio; для него
  не нужны приглашение тестировщика и ручное одобрение администратора. Инструкции включения —
  в `docs/deploy.md`, технические ограничения и статус пилота — в
  `docs/research/zernio.md`.

Архитектура готова к другим каналам Zernio через общую platform-колонку и единый вебхук; сроки
их подключения не определены.
...
/src/lib/kb-templates    — предустановленные шаблоны баз знаний по сферам деятельности ({ru,de}/*.md)
