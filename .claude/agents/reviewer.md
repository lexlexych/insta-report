...
2. Дифф: если оркестратор указал диапазон в задании (например `git diff <base>..HEAD` для тикета
   в эпик-ветке) — ревьюй ТОЛЬКО его; иначе `git diff main...HEAD`. Изучи полностью.
