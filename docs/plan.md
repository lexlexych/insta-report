...
> сферы деятельности (`src/lib/kb-templates/*.md`), который пользователь правит под себя.
...
**Онбординг (v1.2):** первый запуск Mini App открывает полноэкранный визард без навигации разделов, с кнопкой «Далее» внизу (неактивна, пока экран не заполнен). Пользователь выбирает сферу деятельности (организация мероприятий · фотограф · туристическое агентство · салон массажа — список расширяем), вводит название бизнеса и свой Instagram-аккаунт, затем правит под себя предустановленный Markdown-шаблон базы знаний своей сферы (шаблоны — `src/lib/kb-templates/*.md`, с примерными адресами/часами/ценами и блоком «Вопросы и ответы»). LLM в онбординге не используется; system prompt — фиксированный шаблон с подстановкой названия бизнеса.
...
Зависимости: —
Задача: реестр `BUSINESS_SPHERES` (id, иконка, i18n-название ru/de) для сфер events / photographer / travel / massage; типобезопасный доступ `getKbTemplate(id)` к готовым шаблонам `src/lib/kb-templates/*.md` (содержимое согласовано, не менять); кодоген `scripts/gen-kb-templates.ts` → `templates.gen.ts` (коммитится, .md — источник истины), npm-скрипт `gen:kb-templates` с режимом `--check`.
AC: шаблоны всех четырёх сфер доступны из кода; кодоген детерминирован; названия сфер — через i18n.
...
Задача: полноэкранный онбординг без навигации разделов, внизу кнопка «Далее» (disabled до заполнения экрана); экраны: 1) выбор одной сферы деятельности (иконки+названия); 2) название бизнеса + Instagram-аккаунт → ветвление по статусу заявки; 3а) «заявка подана, продолжайте настройку» / 3б) реальное OAuth-подключение (общий компонент `InstagramConnectPanel` с `/app/connect-instagram`); 4) правка предзаполненного шаблона базы знаний, кнопка «Завершить» → `POST /api/miniapp/onboarding/complete` (КБ + статический `defaultSystemPrompt(orgName)` + `seed_default_labels`); 5) финальный экран «что дальше» (симулятор, категории, настройки, дашборд, как приходят карточки) → «Начать работу» → дашборд. Прогресс — в `tenants.onboarding_step`, возврат на незавершённый шаг.
AC: кнопка «Далее» неактивна до валидности экрана; обе ветки экрана 3 работают; шаблон подставляется по сфере; после финала навигация возвращается.
