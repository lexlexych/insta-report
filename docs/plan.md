...
| Один Telegram-бот на всех | Центральный бот @ProductBot; черновики — в личный чат владельца. Форум-группы с топиками по категориям — опциональный эпик |
...
**T-024 · (Опционально) Форум-группа с топиками по категориям**
Зависимости: T-021, T-023
Задача: настройка «Получать черновики в группу»: инструкция (создать супергруппу c topics, добавить бота админом), захват `chat_id` через событие `my_chat_member`; при доставке черновика — `createForumTopic` для метки при отсутствии `tg_thread_id` (сохранить), слать с `message_thread_id`; топик «История» для лога отправленных. Ошибка топиков → fallback в личный чат.
AC: интеграционный тест с моком TG: топик создаётся один раз на метку; fallback работает.
