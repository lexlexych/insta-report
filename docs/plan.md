...
> **⚠️ РЕВИЗИЯ v1.2 (2026-07-11, «Онбординг 2.0», Эпик 10).** LLM-генерация базы знаний при
> онбординге упразднена: база знаний создаётся из предустановленного Markdown-шаблона выбранной
> сферы деятельности, который пользователь правит под себя. Шаблоны хранятся на всех языках
> системы (`src/lib/kb-templates/{ru,de}/*.md`) и показываются на языке интерфейса,
> определяемом из языка Telegram при первом запуске Mini App.
> Онбординг — полноэкранный визард из 5 экранов (сфера → данные бизнеса и Instagram-аккаунт →
> заявка/подключение → правка шаблона → «что дальше»), подключение Instagram встроено в него
> через реестр `ig_accounts` c ручным подтверждением администратора (для пилота аккаунты
> добавляются в наше Meta App вручную). Актуальные тикеты — Эпик 10 (T-035…T-039);
> T-009/T-010 описывают историческую реализацию и упразднены.
...
**Онбординг (v1.2):** первый запуск Mini App открывает полноэкранный визард без навигации разделов, с кнопкой «Далее» внизу (неактивна, пока экран не заполнен). Язык интерфейса определяется из языка Telegram пользователя (`resolveLocale`) и синхронизируется в `tenants.ui_locale`. Пользователь выбирает сферу деятельности (организация мероприятий · фотограф · туристическое агентство · салон массажа — список расширяем), вводит название бизнеса и свой Instagram-аккаунт, затем правит под себя предустановленный Markdown-шаблон базы знаний своей сферы. Шаблоны хранятся на каждом языке системы (`src/lib/kb-templates/{ru,de}/*.md`, с примерными адресами/часами/ценами и блоком «Вопросы и ответы») и подставляются на языке интерфейса. LLM в онбординге не используется; system prompt — фиксированный шаблон с подстановкой названия бизнеса.

**Подключение Instagram (v1.2, пилот):** используется Business Login for Instagram через наше платформенное Meta App (T-033/T-034); до прохождения App Review аккаунты добавляются в Meta App вручную администратором. Введённый на онбординге Instagram-аккаунт сверяется с реестром `ig_accounts`: если аккаунта нет — создаётся заявка со статусом «ожидание подтверждения», администратор (id из `ADMIN_TELEGRAM_IDS`) получает в Telegram сообщение с кнопкой «Подтвердить», добавляет аккаунт в Meta App и подтверждает; пользователю приходит сообщение со ссылкой на экран подключения. Если аккаунт уже подтверждён — OAuth-подключение выполняется прямо в онбординге.
...
Meta POST /api/wh/ig/{tenantId}
 ...
 → отправить в TG карточку (HTML, blockquote pendingText, <pre> черновик, inline-кнопки)
...
Telegram POST /api/telegram (callback_query, data="send:{draft_id}")
 ...
 → Graph API: перечитать беседу; бизнес уже ответил после trigger_ts?
    ...
    → нет: POST /{ig_id}/messages (разбиение текста >1000 байт), status=sent,
           editMessageText карточки на месте → «✅ отправлено в HH:MM», лог в message_log
```
...
tenants            id uuid pk, telegram_user_id bigint unique, tg_chat_id bigint,
                   ...
                   -- v1.2: + business_sphere text (id сферы из src/lib/kb-templates);
                   --       org_description удаляется тикетом T-039
...
ig_accounts        id uuid pk, ig_username text unique (нормализован: lowercase, без @),
                   tenant_id fk null (on delete set null),
                   status text ('pending'|'approved') default 'pending',
                   requested_at, approved_at, approved_by_tg_id bigint
                   -- v1.2 (Эпик 10): реестр Instagram-аккаунтов, добавленных в наше Meta App;
                   -- заявки подтверждает администратор кнопкой в Telegram
```
...
**T-009 · Визард онбординга (UI)** *(упразднён ревизией v1.2 — заменён визардом T-038, Эпик 10)*
...
**T-010 · Генерация базы знаний и system prompt** *(упразднён ревизией v1.2 — шаблоны сфер деятельности вместо LLM-генерации; демонтаж — T-039)*
...
Задача: `src/lib/tg/draftCard.ts`: HTML-рендер (эскейпинг!) — «📩 Новое сообщение от <a>username</a> в {time}» + `<blockquote>` pendingText + `<pre>` черновик (variant='pending'); после отправки та же карточка редактируется на месте в variant='sent' («✅ отправлено в {time} <получатель>», черновик в `<blockquote>`, без кнопок и категории); inline-кнопки: URL `https://ig.me/m/{username}` и callback `send:{draftId}`. Оркестратор `src/lib/pipeline/deliver.ts`: `cancelPendingByConversation` (+ deleteMessage старой карточки, ошибки TG глотать) → sendMessage карточки → insert drafts(pending). `message_log`: входящее.
...
### Эпик 10 — Онбординг 2.0: сферы деятельности, шаблоны и заявки на подключение (ревизия v1.2)

Замена онбординга T-009/T-010: вместо LLM-генерации базы знаний — предустановленные
Markdown-шаблоны по сферам деятельности; подключение Instagram встроено в онбординг через
реестр `ig_accounts` с подтверждением администратором. Полные тексты — `docs/tickets/T-035…T-039.md`.

**T-035 · Реестр сфер деятельности и шаблоны баз знаний**
Зависимости: —
Задача: реестр `BUSINESS_SPHERES` (id, иконка, i18n-название ru/de) для сфер events / photographer / travel / massage; типобезопасный доступ `getKbTemplate(id, locale)` к готовым шаблонам `src/lib/kb-templates/{locale}/*.md` — на каждом языке системы (ru/de; содержимое согласовано, не менять); кодоген `scripts/gen-kb-templates.ts` → `templates.gen.ts` (коммитится, .md — источник истины; падает при неполном комплекте сфера×локаль), npm-скрипт `gen:kb-templates` с режимом `--check`.
AC: шаблоны всех сфер доступны из кода на обеих локалях (4 × 2); кодоген детерминирован; названия сфер — через i18n.

**T-036 · БД: реестр Instagram-аккаунтов (ig_accounts) и сфера деятельности тенанта**
Зависимости: T-034
Задача: миграция `0011_ig_accounts.sql` — таблица `ig_accounts` (см. §2.4) + `tenants.business_sphere`; RLS deny-all; репозиторий `src/lib/db/igAccounts.ts` (`normalizeIgUsername`, `findByUsername`, `createPending`, атомарный `approve`, `getByTenant`, `bindTenant`); правка `types.gen.ts`.
AC: миграция применяется после 0010; normalize отсекает `@`/регистр/невалидные символы; approve идемпотентен.

**T-037 · Заявки на подключение Instagram: проверка, уведомление и подтверждение администратором**
Зависимости: T-005, T-033, T-036
Задача: `POST/GET /api/miniapp/onboarding/ig-account` (проверка username по реестру: нет → pending-заявка + сообщение админам из `ADMIN_TELEGRAM_IDS` с inline-кнопкой «✅ Подтвердить»; approved → сразу approved; чужой аккаунт → 409); grammY-callback `igacc_ok:{id}` (только для админов, идемпотентный) переводит заявку в approved и шлёт пользователю сообщение с кнопкой-ссылкой `t.me/{bot}?startapp=connect` на экран подключения.
AC: полный цикл заявки работает; повторные POST/клики не плодят заявок и уведомлений; не-админ подтвердить не может.

**T-038 · Онбординг 2.0 — полноэкранный визард (UI + финализация)**
Зависимости: T-033, T-035, T-037
Задача: полноэкранный онбординг без навигации разделов, внизу кнопка «Далее» (disabled до заполнения экрана); при первом запуске локаль из языка Telegram синхронизируется в `tenants.ui_locale` — все экраны и шаблон идут на ней; экраны: 1) выбор одной сферы деятельности (иконки+названия); 2) название бизнеса + Instagram-аккаунт → ветвление по статусу заявки; 3а) «заявка подана, продолжайте настройку» / 3б) реальное OAuth-подключение (общий компонент `InstagramConnectPanel` с `/app/connect-instagram`); 4) правка предзаполненного шаблона базы знаний `getKbTemplate(sphere, locale)`, кнопка «Завершить» → `POST /api/miniapp/onboarding/complete` (КБ + статический `defaultSystemPrompt(orgName)` + `seed_default_labels`); 5) финальный экран «что дальше» (симулятор, категории, настройки, дашборд, как приходят карточки) → «Начать работу» → дашборд. Прогресс — в `tenants.onboarding_step`, возврат на незавершённый шаг.
AC: кнопка «Далее» неактивна до валидности экрана; обе ветки экрана 3 работают; шаблон подставляется по сфере и языку интерфейса (ru-Telegram → ru, иначе de); после финала навигация возвращается.

**T-039 · Демонтаж LLM-генерации онбординга**
Зависимости: T-038
Задача: удалить `POST /api/miniapp/onboarding/generate`, `kbGenerationPrompt`, остатки старого визарда и осиротевшие i18n-ключи; миграция `0012_drop_org_description.sql` (drop `tenants.org_description`) + `types.gen.ts`; актуализировать README/доки.
AC: grep по `org_description|kbGenerationPrompt|onboarding/generate` пуст; настройки/симулятор/пайплайн без регрессий.
