...
## 9. Итоги реализации и вопросы пилота

`docs/manual-tests/T-047.md`…`T-051.md` содержат воспроизводимые сценарии, но в репозитории
нет отчёта о фактических прогонах с внешними аккаунтами. Поэтому ниже зафиксировано поведение,
закрытое реализацией, а сведения, требующие наблюдения Zernio/Meta, помечены **«не проверено в
пилоте»**. Перед массовым включением оператор должен дописать результат конкретного прогона.

1. **Отказ OAuth.** Callback без `accountId` безопасно считается неуспехом: пользователь видит
   страницу ошибки или возвращается на `/app?zernio=error`, а подключение не активируется.
   Конкретные параметры, которые Zernio добавляет при отказе, не сохраняются и **не проверены в
   пилоте**; сценарий отмены есть в T-051.
2. **`message.sent` и ручной ответ.** Вебхук принимает исходящие события, а отправленный через
   карточку ID предварительно регистрируется, чтобы собственный echo не создал новый черновик.
   Перед отправкой дополнительно выполняется anti-double-check истории переписки для ручного
   ответа. Приходит ли `message.sent` именно для ручного ответа из приложения Instagram —
   **не проверено в пилоте**; это проверяется шагами 3–4 T-050.
3. **Inbox addon и лимиты Inbox.** Код возвращает ошибку Zernio пользователю и не раскрывает
   ключ. Необходимость addon и действующие rate limits для выбранного тарифа **не проверены в
   пилоте**. Симптом — `403 Inbox addon required`; действие оператора — включить addon и
   повторить операцию (см. deploy.md).
4. **Тип Instagram-аккаунта.** Документация и сценарии требуют Business-аккаунт. Работа
   Creator-аккаунтов через Zernio **не проверена в пилоте**; не предлагайте этот путь владельцу
   Creator-аккаунта без отдельной проверки.
5. **Латентность входящего сообщения.** `message.received` передаётся в общий pipeline и
   дедуплицируется по Meta mid, но сравнение задержки с прямым Meta webhook **не проверено в
   пилоте**. При прогоне измерьте время отправки DM и появления Telegram-карточки для обоих
   путей.
6. **Лимит профилей.** На `403` при создании profile подключение корректно завершается ошибкой,
   без частичной записи второго профиля. Численный лимит текущего плана **не проверен в пилоте**;
   он отображается в Zernio Dashboard и описан в deploy.md.
